...
let userId = null;
firebase.auth().onAuthStateChanged(user => { if(user) userId = user.uid; });
...
document.getElementById("analyzeBtn").onclick = async ()=>{
  const selected = [...document.querySelectorAll(".symptom.selected")].map(e=>e.textContent.toLowerCase());
  if(selected.length===0){document.getElementById("output").innerHTML="<p class='small-muted'>Select at least one symptom</p>";return;}
  let result={diagnosis:"Unknown",medicine:"N/A",dosage:"N/A",frequency:"N/A",duration:"N/A",warning:"Consult a doctor"};
  for(let c of conditions){const matches=c.keywords.filter(k=>selected.includes(k.toLowerCase())); if(matches.length>=2){result=c;break;}}
  document.getElementById("output").innerHTML=`<div class="result"><h3>Diagnosis: ${result.diagnosis}</h3><p><strong>Medicine:</strong> ${result.medicine}</p><p><strong>Dosage:</strong> ${result.dosage}</p><p><strong>Frequency:</strong> ${result.frequency}</p><p><strong>Duration:</strong> ${result.duration}</p><p class="warning">⚠️ ${result.warning}</p></div>`;
  try{await db.collection("prescriptionResults").add({userId,userSymptoms:selected,diagnosis:result.diagnosis,medicine:result.medicine,createdAt:firebase.firestore.FieldValue.serverTimestamp()});showToast("Saved successfully","#28a745");}catch(e){console.error(e);showToast("Failed to save","#dc3545");}
};

// Toast
function showToast(msg,color="#007bff"){const t=document.getElementById("toast");t.textContent=msg;t.style.background=color;t.classList.add("show");setTimeout(()=>t.classList.remove("show"),3000);}
